...
public class Circle {
    ...
    public Circle(GLSurfaceView mv) {//构造器
        // 初始化顶点坐标与着色数据
        initVertexData();
        // 初始化shader
        initShader(mv);
    ...
    public void initShader(GLSurfaceView mv) {
        // 加载顶点着色器的脚本内容
        mVertexShader = ShaderUtil.loadFromAssetsFile("book/sample5/vertex.vsh",
                mv.getResources());
        // 加载片元着色器的脚本内容
        mFragmentShader = ShaderUtil.loadFromAssetsFile("book/sample5/frag.fsh",
                mv.getResources());
        // 基于顶点着色器与片元着色器创建程序
