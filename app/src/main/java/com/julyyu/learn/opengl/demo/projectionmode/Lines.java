...
class Lines {
    ...
    public Lines(GLSurfaceView mv, float z) {
        this.z = z;
    ...
    public void setOSize(Size size) {
        this.size = size;
        Matrix.orthoM(
                mProjMatrix,    //存储生成矩阵元素的float[]类型数组
                0,                //填充起始偏移量
                -1 * scale, 1* scale,    //near面的left、right
                -1 * scale, 1 * scale,    //near面的bottom、top
    ...
    public void setPSize(Size size) {
        scale = 0.5f;
        this.size = size;
        Matrix.frustumM(
                mProjMatrix,    //存储生成矩阵元素的float[]类型数组
