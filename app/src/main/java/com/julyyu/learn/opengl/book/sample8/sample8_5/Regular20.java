package com.julyyu.learn.opengl.book.sample8.sample8_5;

import android.opengl.GLES30;
import android.opengl.GLSurfaceView;

import com.julyyu.learn.opengl.MatrixState;
import com.julyyu.learn.opengl.book.sample8.VectorUtil;
...
public class Regular20 {
    ...
    public Regular20(GLSurfaceView mv, float scale, float aHalf, int n) {
    ...
    public void initVertexData(float scale, float aHalf, int n) {//自定义的初始化顶点数据的方法
        ...
        for (int k = 0; k < vertices20.length; k += 9)//对正二十面体中的每个三角形循环
        {
            ...
            for (int i = 0; i <= n; i++) {//根据切分的份数求出几何球原始顶点的坐标
            ...
            for (int i = 0; i < n; i++) {//循环生成构成几个球各个三角形的顶点编号列表
                ...
                for (int j = 0; j < viCount - 1; j++) {//前面的四边形
    ...
    public void initAlVertix20(ArrayList<Float> alVertix20, float aHalf, float bHalf) {
        ...
        alVertix20.add(-bHalf);//对应图8-17的1号点

        alVertix20.add(0f);
        alVertix20.add(aHalf);
        alVertix20.add(bHalf);//对应图8-17的2号点
        alVertix20.add(aHalf);
        alVertix20.add(bHalf);
        alVertix20.add(0f);//对应图8-17的3号点
        alVertix20.add(bHalf);
        alVertix20.add(0f);
        alVertix20.add(-aHalf);//对应图8-17的4号点
        alVertix20.add(-bHalf);
        alVertix20.add(0f);
        alVertix20.add(-aHalf);//对应图8-17的5号点
        alVertix20.add(-aHalf);
        alVertix20.add(bHalf);
        alVertix20.add(0f);//对应图8-17的6号点

        alVertix20.add(-bHalf);
        alVertix20.add(0f);
        alVertix20.add(aHalf);//对应图8-17的7号点
        alVertix20.add(bHalf);
        alVertix20.add(0f);
        alVertix20.add(aHalf);//对应图8-17的8号点
        alVertix20.add(aHalf);
        alVertix20.add(-bHalf);
        alVertix20.add(0f);//对应图8-17的9号点
        alVertix20.add(0f);
        alVertix20.add(-aHalf);
        alVertix20.add(-bHalf);//对应图8-17的10号点
        alVertix20.add(-aHalf);
        alVertix20.add(-bHalf);
        alVertix20.add(0f);//对应图8-17的11号点

        alVertix20.add(0f);
        alVertix20.add(-aHalf);
        alVertix20.add(bHalf);//对应图8-17的12号点
    ...
    public void initShader(GLSurfaceView mv) {
        //加载顶点着色器的脚本内容
        mVertexShader = ShaderUtil.loadFromAssetsFile("book/sample8/vertex_tex_light.vsh", mv.getResources());
        //加载片元着色器的脚本内容
        mFragmentShader = ShaderUtil.loadFromAssetsFile("book/sample8/frag_tex_light.fsh", mv.getResources());
        //基于顶点着色器与片元着色器创建程序
