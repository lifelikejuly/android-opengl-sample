package com.julyyu.learn.opengl.book.sample5.sample5_2;
 ...
 */
public class SixStartGLRenderer2 extends GLSurfaceView {
    ...
    private boolean defluat = false;

    public SixStartGLRenderer2(Context context) {
        super(context);
    ...
    public SixStartGLRenderer2(Context context, boolean defluat) {
        super(context);
        this.defluat = defluat;
        this.setEGLContextClientVersion(3); //设置使用OPENGL ES3.0
        mRenderer = new SceneRenderer();    //创建场景渲染器
        setRenderer(mRenderer);                //设置渲染器
        setRenderMode(GLSurfaceView.RENDERMODE_CONTINUOUSLY);//设置渲染模式为主动渲染
    ...
    private class SceneRenderer implements Renderer {
        ...
        public void onSurfaceCreated(GL10 gl, EGLConfig config) {
            ...
            for (int i = 0; i < ha.length; i++) {
                if(defluat){
                    ha[i] = new SixPointedStar(SixStartGLRenderer2.this, 0.2f, 0.5f, -0.3f * i);
                }else{
                    ha[i] = new SixPointedStar(SixStartGLRenderer2.this, 0.4f, 1.0f, -1.0f * i);
                }
