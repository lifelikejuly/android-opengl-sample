...
import static com.julyyu.learn.opengl.util.GLESUtils.createProgram;
...
{	
    ...
    public Cloud(TextureMapMoreMixRenderer mv, float r)
    {    	
    	//初始化顶点数据
    	initVertexData(r);
    	//初始化着色器        
    ...
    //初始化着色器
    public void initShader(TextureMapMoreMixRenderer mv)
    {
    	//加载顶点着色器的脚本内容       
        mVertexShader= ShaderUtil.loadFromAssetsFile("book/sample10/vertex_cloud.sh", mv.getResources());
        ShaderUtil.checkGlError("==ss==");   
        //加载片元着色器的脚本内容
        mFragmentShader=ShaderUtil.loadFromAssetsFile("book/sample10/frag_cloud.sh", mv.getResources());
        //基于顶点着色器与片元着色器创建程序
        ShaderUtil.checkGlError("==ss==");      
