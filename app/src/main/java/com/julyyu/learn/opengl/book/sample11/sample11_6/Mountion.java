...
public class Mountion {
    ...
    public Mountion(SkyDomeRenderer mv, float[][] yArray, int rows, int cols) {
    ...
    //初始化着色器的方法
    public void initShader(SkyDomeRenderer mv) {
        String mVertexShader = ShaderUtil.loadFromAssetsFile("book/sample11/vertex_mountion.sh", mv.getResources());
        String mFragmentShader = ShaderUtil.loadFromAssetsFile("book/sample11/frag_mountion.sh", mv.getResources());
        //基于顶点着色器与片元着色器创建程序
